...
def gateway(**kwargs):
    return StripeGateway(Settings(_env_file=None, STRIPE_SECRET_KEY="sk_test_example", **kwargs))
...
@pytest.mark.parametrize("key,live", [("", False), ("sk_live_example", False), ("sk_test_example", True)])
def test_missing_or_wrong_mode_api_key(key, live):
    provider = StripeGateway(Settings(_env_file=None, STRIPE_SECRET_KEY=key, STRIPE_LIVE_MODE=live))
...
async def test_sdk_calls_use_durable_idempotency_keys(monkeypatch):
    ...
    for endpoint in [client.v1.checkout.sessions.create, client.v1.checkout.sessions.retrieve,
                     client.v1.checkout.sessions.expire, client.v1.refunds.create, client.v1.refunds.retrieve]:
        endpoint.return_value.to_dict.return_value = {"id": "example"}
    assert await provider.create_checkout({"mode": "payment"}, "stable-key") == {"id": "example"}
    client.v1.checkout.sessions.create.assert_called_once_with({"mode": "payment"}, options={"idempotency_key": "stable-key"})
    ...
    client.v1.refunds.create.assert_called_once_with({"payment_intent": "pi_1", "amount": 100},
...
async def test_definite_checkout_rejection_is_distinct_from_timeout():
    with pytest.raises(CheckoutRejectedError):
        await gateway()._call(Mock(side_effect=stripe.InvalidRequestError("private", "payment_method_types")),
...
async def test_find_checkout_for_payment_intent(monkeypatch, items):
    ...
    monkeypatch.setattr(provider, "client", lambda: client)
    assert await provider.checkout_for_payment_intent("pi_test") == (items[0] if items else None)
    client.v1.checkout.sessions.list.assert_called_once_with({"payment_intent": "pi_test", "limit": 1})
...
def test_signature_verification():
    ...
    assert provider.verify_event(payload, signature(payload, int(time.time())))["id"] == "evt_1"
    for body, header in [(payload, ""), (payload, signature(payload, 1)),
...
async def test_payment_email_queue_worker_and_template(monkeypatch):
    ...
    assert publish.call_args.kwargs["args"] == ("payment", "user@example.com",
