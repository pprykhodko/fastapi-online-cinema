...
def test_checkout_lines_preserve_amounts_and_provider_limit(count):
    order = SimpleNamespace(
        ...
        items=[
            SimpleNamespace(
                price_at_order=Decimal("12.34"),
                movie=SimpleNamespace(name=f"Movie {i}")
            )
            for i in range(count)
        ]
    )
    result = build_checkout_data(
        order, "eur", "https://example.com/return/?lang=en#result", "key", 123456
    )
    ...
    assert parse_qs(urlsplit(result["success_url"]).query) == {
        "lang": ["en"],
...
def test_checkout_amount_respects_quantities():
    assert (
        checkout_amount(
            {"line_items": [{"quantity": 2, "price_data": {"unit_amount": 99}}]}
        )
        == 198
    )
