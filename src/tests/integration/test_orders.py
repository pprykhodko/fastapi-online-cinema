...
from src.database.models import (
    CartItemModel, CartModel, CertificationModel, MovieModel, OrderItemModel,
...
async def orders_api(login_api):
    ...
    async with sessions() as db:
        ...
        moderator = UserModel(
            email="mod@example.com", group_id=moderator_group.id, _hashed_password="unused", is_active=True,
        )
        db.add_all([admin, moderator])
        movie = MovieModel(
            id=1, name="Movie", year=2020, time=90, imdb=8, votes=5,
            description="Story", price=Decimal("20"), certification=CertificationModel(name="PG"),
        )
        db.add(movie)
        ...
        for order_id, owner, order_status, date in [
            ...
            db.add(OrderModel(
                id=order_id, user_id=owner, status=order_status,
                created_at=datetime.fromisoformat(date).replace(tzinfo=timezone.utc),
        ...
        headers = {
            ...
            for role, account_id in [
                ("user", user_id), ("other", other.id), ("admin", admin.id), ("moderator", moderator.id),
            ]
        }
...
async def test_own_orders_are_paginated_and_private(orders_api):
    ...
    result = await client.get(URL, headers=headers["user"], params={"page": 1, "per_page": 2})
    ...
    page2 = await client.get(URL, headers=headers["user"], params={"page": 2, "per_page": 2})
    assert [order["id"] for order in page2.json()["items"]] == [1]
    assert (await client.get(URL, headers=headers["admin"])).json()["items"] == []
    assert (await client.get(URL, headers=headers["user"], params={"page": 99})).json()["items"] == []
...
@pytest.mark.parametrize("method,path", [
    ("GET", "4/"), ("PATCH", "4/cancel/"), ("GET", "999/"), ("PATCH", "999/cancel/"),
])
...
async def test_cancel_preserves_items_and_cart_and_can_order_again(orders_api):
    client, sessions, _, _, headers = orders_api
    # Remove the paid-order fixture from this scenario: this movie must not be purchased.
    async with sessions() as db:
    ...
    assert (await client.patch(URL + "1/cancel/", headers=headers["user"])).status_code == 409
    cart = await client.get("/api/v1/cart/", headers=headers["user"])
    assert len(cart.json()["items"]) == 1
    assert (await client.post("/api/v1/cart/checkout/", headers=headers["user"])).status_code == 201
...
async def test_only_pending_order_can_be_canceled(orders_api, order_id):
    ...
    assert (await client.get(URL + f"{order_id}/", headers=headers["user"])).json() == before


@pytest.mark.asyncio
@pytest.mark.parametrize("payment_status,expected", [
    (PaymentStatusEnum.SUCCESSFUL, 409), (PaymentStatusEnum.REFUNDED, 409), (PaymentStatusEnum.CANCELED, 200),
])
async def test_payment_record_prevents_cancellation_even_if_order_status_is_stale(orders_api, payment_status, expected):
    client, sessions, user_id, _, headers = orders_api
    async with sessions() as db:
        db.add(PaymentModel(user_id=user_id, order_id=1, status=payment_status, amount=Decimal("1.25")))
        await db.commit()
    response = await client.patch(URL + "1/cancel/", headers=headers["user"])
...
@pytest.mark.parametrize("params,ids", [
    ({}, [4, 3, 2, 1]), ({"status": "pending"}, [4, 1]),
    ({"status": "paid"}, [2]), ({"status": "canceled"}, [3]),
    ({"date_from": "2030-01-02", "date_to": "2030-01-02"}, [2]),
    ({"date_from": "2030-01-03"}, [4, 3]), ({"date_to": "2030-01-01"}, [1]),
    ({"date_from": "2031-01-01"}, []), ({"user_id": 9999}, []),
...
async def test_admin_combines_filters_and_pagination(orders_api):
    client, _, user_id, _, headers = orders_api
    result = await client.get(URL + "admin/", headers=headers["admin"], params={
        "user_id": user_id, "date_from": "2030-01-01", "date_to": "2030-01-03", "per_page": 1, "page": 2,
    })
    assert result.status_code == 200
    assert result.json()["total"] == 3
    assert result.json()["items"][0]["id"] == 2
    result = await client.get(URL + "admin/", headers=headers["admin"], params={
        "user_id": user_id, "status": "pending", "date_from": "2030-01-02",
    })
...
@pytest.mark.asyncio
@pytest.mark.parametrize("method,path", [
    ("GET", ""), ("GET", "admin/"), ("GET", "1/"), ("PATCH", "1/cancel/"),
])
...
async def test_orders_require_active_authentication(orders_api, method, path, inactive):
    ...
    if inactive:
        async with sessions() as db:
            ...
            await db.commit()
    response = await client.request(method, URL + path, headers=headers["user"] if inactive else {})
    assert response.status_code == (403 if inactive else 401)


@pytest.mark.asyncio
@pytest.mark.parametrize("params", [
    {"date_from": "2030-02-01", "date_to": "2030-01-01"}, {"date_to": "invalid"},
    {"user_id": 0}, {"user_id": 2**31}, {"status": "invalid"}, {"page": 0}, {"per_page": 101}, {"extra": 1},
])
async def test_admin_filter_validation(orders_api, params):
    client, _, _, _, headers = orders_api
    assert (await client.get(URL + "admin/", headers=headers["admin"], params=params)).status_code == 422


@pytest.mark.asyncio
@pytest.mark.parametrize("params", [
    {"page": 0}, {"per_page": 0}, {"per_page": 101}, {"user_id": 2}, {"status": "paid"},
])
async def test_own_list_cannot_override_owner(orders_api, params):
    client, _, _, _, headers = orders_api
    assert (await client.get(URL, headers=headers["user"], params=params)).status_code == 422
...
async def test_order_path_validation(orders_api, order_id):
    client, _, _, _, headers = orders_api
    assert (await client.get(URL + order_id + "/", headers=headers["user"])).status_code == 422
...
async def test_order_read_failure_is_safe(orders_api, monkeypatch, path, method):
    client, _, _, _, headers = orders_api
    monkeypatch.setattr(OrderRepository, method, AsyncMock(side_effect=OperationalError("private", {}, Exception())))
...
def test_orders_openapi_contract():
    ...
    for suffix, method in [("", "get"), ("admin/", "get"), ("{order_id}/", "get"), ("{order_id}/cancel/", "patch")]:
...
@pytest.mark.parametrize("case", [
    "valid", "free", "paid", "canceled", "payment", "empty", "purchased", "deleted", "unpriced", "price", "total",
])
async def test_pre_payment_validation_preserves_snapshots(orders_api, case):
    ...
    async with sessions() as db:
        ...
        if case in {"valid", "free"}:
            ...
            assert order.total_amount == (Decimal("0") if case == "free" else Decimal("1.25"))
            assert db.in_transaction()  # The payment caller still owns this transaction.
        else:
        ...
        await db.rollback()
    async with sessions() as db:
        item = await db.scalar(select(OrderItemModel).where(OrderItemModel.order_id == 1))
        if case != "empty":
            assert item.price_at_order == (Decimal("0") if case == "free" else Decimal("1.25"))
