...
class FakeStripe(StripeGateway):
    ...
    async def create_checkout(self, data, key):
        ...
        session = self.sessions.setdefault(
            ...
            {
                "id": f"cs_test_{key}",
                "url": f"https://checkout.stripe.com/{key}",
        ...
        return deepcopy(
            next(
                (
                    session
                    for session in self.sessions.values()
                    if session["payment_intent"] == payment_intent
                ),
                None
            )
        )
    ...
    async def expire_checkout(self, session_id):
        session = next(
            session for session in self.sessions.values() if session["id"] == session_id
        )
    ...
    async def refund(self, payment_intent, amount, key):
        self.refund_calls += 1
        return self.refunds.setdefault(
            "re_1",
            {
                "id": "re_1",
...
async def payments_api(login_api, monkeypatch):
    ...
    async with sessions() as db:
        ...
        db.add(
            CartModel(
                user_id=user_id, items=[CartItemModel(movie=movie) for movie in movies]
            )
        )
        db.add(
            OrderModel(
        ...
        await db.commit()
        headers = {
            role: {"Authorization": f"Bearer {manager.create_access_token(account.id)}"}
            for role, account in [
                ("user", user),
                ("other", other),
                ("admin", admin),
...
async def start_payment(api):
    ...
    response = await client.post(
        URL + "checkout/", json={"order_id": 1}, headers=headers["user"]
    )
...
async def test_checkout_reuses_session_and_does_not_pay_order(payments_api):
    ...
    assert (
        await client.get(URL + "return/", params={"session_id": session["id"]})
    ).status_code == 200
    async with sessions() as db:
...
async def test_success_is_atomic_and_duplicate_safe(payments_api):
    ...
    assert (
        await post_event(client, expired, "checkout.session.expired")
    ).status_code == 200
    async with sessions() as db:
    ...
    assert (await client.get(URL + "purchased/", headers=headers["user"])).json()[
        "items"
    ][0]["id"] == 1
    assert (
        "Payment successful"
...
@pytest.mark.parametrize(
    "method,path,body",
    [
        ("POST", "checkout/", {"order_id": 1}),
        ("POST", "checkout/1/cancel/", None),
        ("POST", "refund/", {"payment_id": 1}),
        ("GET", "1/", None)
    ]
)
async def test_other_users_cannot_access_payment(payments_api, method, path, body):
    ...
    for role in ["other", "admin", "mod"]:
        response = await client.request(
            method, URL + path, json=body, headers=headers[role]
        )
...
async def test_authentication_required(payments_api, path, method):
    assert (
        await payments_api[0].request(method, URL + path, json={"order_id": 1})
    ).status_code == 401
...
async def test_invalid_filters(payments_api, params):
    assert (
        await payments_api[0].get(
            URL + "admin/", headers=payments_api[2]["admin"], params=params
        )
    ).status_code == 422
...
async def test_cancel_expires_session_and_preserves_cart(payments_api):
    ...
    session = await start_payment(payments_api)
    assert (
        await client.patch("/api/v1/orders/1/cancel/", headers=headers["user"])
    ).status_code == 409
    for _ in range(2):
        assert (
            await client.post(URL + "checkout/1/cancel/", headers=headers["user"])
        ).status_code == 200
    assert (
        await post_event(client, session, "checkout.session.expired")
    ).status_code == 200
    async with sessions() as db:
        assert (await db.get(OrderModel, 1)).status == OrderStatusEnum.CANCELED
        assert await db.scalar(select(func.count()).select_from(CartItemModel)) == 2
        assert await db.scalar(select(func.count()).select_from(PaymentModel)) == 1
    assert (await client.get(URL, headers=headers["user"])).json()["items"][0][
        "status"
    ] == "canceled"
...
async def test_mismatched_signed_session_does_not_fulfill(payments_api, field, value):
    session = await start_payment(payments_api)
    data = dict(
        session, status="complete", payment_status="paid", payment_intent="pi_test"
    )
    data[field] = value
    assert (await post_event(payments_api[0], data)).status_code == 400
    assert (await payments_api[0].get(URL, headers=payments_api[2]["user"])).json()[
        "total"
    ] == 0
...
async def test_unsigned_and_wrong_mode_webhooks_rejected(payments_api):
    ...
    assert (
        await client.post(URL + "webhook/", content=b"x" * (1024 * 1024 + 1))
    ).status_code == 413
...
async def test_full_refund_is_idempotent_and_revokes_purchase(payments_api):
    ...
    for _ in range(2):
        response = await client.post(
            URL + "refund/", json={"payment_id": 1}, headers=headers["user"]
        )
        assert response.status_code == 200, response.text
    ...
    assert (
        await post_event(client, next(iter(gateway.sessions.values())))
    ).status_code == 200
    assert (await client.get(URL + "purchased/", headers=headers["user"])).json()[
        "total"
    ] == 0
    assert (
        await client.post(
            "/api/v1/cart/items/", json={"movie_id": 1}, headers=headers["user"]
        )
    ).status_code == 201
...
async def test_pending_refund_waits_for_confirmation(payments_api):
    ...
    assert (
        await post_event(client, gateway.refunds["re_1"], "refund.updated")
    ).status_code == 200
    assert (await client.get(URL + "purchased/", headers=headers["user"])).json()[
        "total"
    ] == 0
...
@pytest.mark.asyncio
async def test_webhook_can_recover_session_id_not_saved_after_timeout(
        payments_api, monkeypatch
):
    client, _, headers, gateway, _ = payments_api
    original = gateway.create_checkout
...
@pytest.mark.asyncio
async def test_email_broker_outage_does_not_rollback_payment_and_retry_recovers(
        payments_api
):
    client, sessions, headers, _, emails = payments_api
    session = await start_payment(payments_api)
...
async def test_webhook_rollback_preserves_order_and_cart(payments_api, monkeypatch):
    ...
    monkeypatch.setattr(
        CartRepository,
        "remove_items",
        AsyncMock(side_effect=OperationalError("query", {}, Exception()))
    )
    assert (await post_event(client, session)).status_code == 503
    async with sessions() as db:
    ...
    assert (await post_event(client, session)).status_code == 200
    assert (await client.get(URL + "1/", headers=headers["user"])).json()[
        "status"
    ] == "successful"


@pytest.mark.asyncio
async def test_canceled_payment_cannot_be_refunded_and_page_shows_cancellation(
        payments_api
):
    client, _, headers, _, _ = payments_api
    session = await start_payment(payments_api)
...
async def test_free_order_does_not_require_card_charge(payments_api):
    ...
    async with sessions() as db:
        ...
        (
            await db.scalar(select(OrderItemModel).where(OrderItemModel.order_id == 1))
...
async def test_unsigned_return_query_cannot_mark_payment_paid(payments_api):
    ...
    assert (
        "processing"
        in (await client.get(URL + "return/", params={"session_id": "cs_forged"})).text
    )
    async with sessions() as db:
        assert (await db.get(OrderModel, 1)).status == OrderStatusEnum.PENDING
...
async def test_refund_event_before_payment_requests_retry(payments_api):
    ...
    assert (
        await post_event(
            payments_api[0],
            {"id": "re_1", "payment_intent": "pi_test"},
            "refund.updated"
        )
    ).status_code == 503
    assert (
        await post_event(payments_api[0], {"id": "re_1"}, "refund.updated")
    ).status_code == 200
...
@pytest.mark.parametrize(
    "field,value",
    [("amount", 1000), ("currency", "eur"), ("payment_intent", "pi_other")]
)
...
async def test_unrelated_checkout_events_are_ignored(payments_api):
    ...
    for data in [
        {},
        {"metadata": {"order_id": "999", "request_key": "other"}},
...
@pytest.mark.asyncio
async def test_incomplete_payment_not_fulfilled_and_missing_intent_rejected(
        payments_api
):
    client, _, headers, _, _ = payments_api
    session = await start_payment(payments_api)
...
async def test_expired_uncertain_request_not_recreated(payments_api, monkeypatch):
    ...
    monkeypatch.setattr(
        gateway,
        "create_checkout",
        AsyncMock(side_effect=HTTPException(status_code=503, detail="Timeout"))
    )
    await client.post(URL + "checkout/", json={"order_id": 1}, headers=headers["user"])
    async with sessions() as db:
...
async def test_no_checkout_to_cancel(payments_api):
    assert (
        await payments_api[0].post(
            URL + "checkout/1/cancel/", headers=payments_api[2]["user"]
        )
    ).status_code == 404
...
async def test_refunded_return_page(payments_api):
    session = await complete_payment(payments_api)
    await payments_api[0].post(
        URL + "refund/", json={"payment_id": 1}, headers=payments_api[2]["user"]
    )
    assert (
        "Payment refunded"
        in (
            await payments_api[0].get(
                URL + "return/", params={"session_id": session["id"]}
            )
        ).text
    )
...
@pytest.mark.asyncio
async def test_definite_rejection_releases_order_but_timeout_does_not(
        payments_api, monkeypatch
):
    client, sessions, headers, gateway, _ = payments_api
    monkeypatch.setattr(
        gateway, "create_checkout", AsyncMock(side_effect=CheckoutRejectedError())
    )
    result = await client.post(
        URL + "checkout/", json={"order_id": 1}, headers=headers["user"]
    )
...
@pytest.mark.parametrize(
    "path,data",
    [
        ("checkout/", {"order_id": 10**30}),
        ("refund/", {"payment_id": 10**30}),
        ("checkout/", {"order_id": 1, "amount": 1})
    ]
)
async def test_client_cannot_override_amount_or_overflow_identifiers(
        payments_api, path, data
):
    assert (
        await payments_api[0].post(
            URL + path, json=data, headers=payments_api[2]["user"]
        )
    ).status_code == 422
...
async def test_checkout_retry_recovers_missed_webhook(payments_api, paid):
    ...
    assert response.status_code == 409
    async with sessions() as db:
        assert (await db.get(OrderModel, 1)).status == (
            OrderStatusEnum.PAID if paid else OrderStatusEnum.CANCELED
        )
        assert await db.scalar(select(func.count()).select_from(PaymentModel)) == 1
        assert await db.scalar(select(func.count()).select_from(CartItemModel)) == (
            1 if paid else 2
        )
    assert gateway.create_calls == 1
    ...
    assert (
        await post_event(
            client,
            session,
            "checkout.session.completed" if paid else "checkout.session.expired"
        )
    ).status_code == 200
...
async def test_unrelated_refund_is_acknowledged_not_retried(payments_api):
    assert (
        await post_event(
            payments_api[0],
            {"id": "re_other", "payment_intent": "pi_other"},
            "refund.updated"
        )
    ).status_code == 200
...
async def test_foreign_metadata_cannot_overflow_order_id(payments_api, order_id):
    assert (
        await post_event(
            payments_api[0],
            {"metadata": {"order_id": order_id, "request_key": "other"}}
        )
    ).status_code == 200
...
async def test_multiple_stripe_line_items_match_stored_order_total(payments_api):
    ...
    async with sessions() as db:
        ...
        assert [
            item["price_data"]["product_data"]["name"] for item in data["line_items"]
        ] == ["Movie 1", "Movie 2"]
    assert (await client.get(URL + "purchased/", headers=headers["user"])).json()[
        "total"
    ] == 2
...
async def test_preexisting_aggregated_checkout_still_validates(payments_api):
    ...
    async with sessions() as db:
        ...
        data["line_items"][0]["price_data"]["product_data"][
            "name"
        ] = "Online Cinema order #1"
