...
async def seed_order(
    sessions, user_id, movie_ids, order_status=OrderStatusEnum.PAID, payment_status=None
):
    async with sessions() as db:
        order = OrderModel(
            user_id=user_id,
            ...
            items=[
                OrderItemModel(movie_id=movie_id, price_at_order=Decimal("10"))
...
async def test_cart_lifecycle_and_isolation(cart_api):
    ...
    for auth in (headers, other_headers):
        ...
        assert (movie["name"], movie["year"], movie["price"]) == (
            "Movie 1",
            2020,
            "10.25"
        )
    ...
    assert (
        await client.post(URL + "items/", headers=headers, json={"movie_id": 2})
    ).status_code == 201
...
async def test_unavailable_movie_cannot_be_added(cart_api, movie_id, expected):
    ...
    response = await client.post(
        URL + "items/", headers=headers, json={"movie_id": movie_id}
    )
...
@pytest.mark.parametrize(
    "order_status,payment_status,owner,expected",
    [
...
async def test_purchase_check_is_user_specific(
        cart_api, order_status, payment_status, owner, expected
):
    client, sessions, user_id, other_id, headers, _ = cart_api
    await seed_order(
        sessions,
        user_id if owner == "self" else other_id,
        [1],
        order_status,
        payment_status
    )
...
async def test_cart_validates_input(cart_api, payload):
    client, _, _, _, headers, _ = cart_api
    assert (
        await client.post(URL + "items/", headers=headers, json=payload)
    ).status_code == 422
...
async def test_cart_requires_active_authentication(cart_api, method, path, inactive):
    ...
    response = await client.request(
        method, URL + path, headers=headers if inactive else {}, json={"movie_id": 1}
    )
...
async def test_only_admin_can_view_other_carts(cart_api, role, expected):
    ...
    if expected == 200:
        ...
        assert (
            await client.get(URL + "users/9999/", headers=headers)
        ).status_code == 404
...
async def test_checkout_creates_snapshot_and_keeps_cart_until_payment(cart_api):
    ...
    async with sessions() as db:
        price = await db.scalar(
            select(OrderItemModel.price_at_order).where(OrderItemModel.movie_id == 1)
        )
...
async def test_checkout_excludes_purchased_and_unavailable_movies(cart_api, keep_valid):
    ...
    assert len((await client.get(URL, headers=headers)).json()["items"]) == (
        1 if keep_valid else 0
    )
...
async def test_cart_unique_conflict_returns_409(cart_api, monkeypatch):
    ...
    monkeypatch.setattr(
        CartRepository,
        "add_item",
        AsyncMock(side_effect=IntegrityError("private", {}, Exception()))
    )
    response = await client.post(URL + "items/", headers=headers, json={"movie_id": 1})
...
def test_cart_openapi_contract():
    ...
    for path, methods in {
        URL: {"get", "delete"},
        URL + "items/": {"post"},
        URL + "items/{movie_id}/": {"delete"},
        URL + "users/{user_id}/": {"get"},
        URL + "checkout/": {"post"}
    }.items():
...
async def test_checkout_pending_overlap_and_exclusion_rollback(cart_api, target):
    ...
    await seed_order(
        sessions,
        other_id if target == "other" else user_id,
        [2],
...
async def test_missing_cart_does_not_create_another_cart_on_read(cart_api):
    ...
    async with sessions() as db:
        assert (
            await db.scalar(select(CartModel).where(CartModel.user_id == user_id))
            is None
        )
