...
async def test_queue_failure_preserves_saved_comment_activity(
        comments_api, monkeypatch, event
):
    ...
    async def check_saved_activity(kind, email, data):
        async with sessions() as db:
            ...
            assert (
                await db.scalar(select(model).where(model.user_id == user_id))
                is not None
            )
    ...
    if event == "reply":
        response = await client.post(
            PATH, headers=headers, json={"content": "Reply", "parent_id": 1}
        )
    else:
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "method,path", [("POST", PATH), ("PUT", LIKE), ("DELETE", LIKE)]
)
...
async def test_authentication(comments_api, method, path, auth):
    ...
    result = await client.request(method, path, headers=headers, json={"content": "Hi"})
...
async def test_unavailable_movie(comments_api, deleted):
    ...
    assert (
        await client.post(path, headers=headers, json={"content": "Hi"})
    ).status_code == 404
...
async def test_database_failure_rolls_back(
    ...
    client, sessions, headers, sender, _ = comments_api
    monkeypatch.setattr(
        CommentRepository, operation, AsyncMock(side_effect=SQLAlchemyError("private"))
    )
    result = await client.request(
        method, path, headers=headers, json={"content": "Hi", "parent_id": 1}
    )
