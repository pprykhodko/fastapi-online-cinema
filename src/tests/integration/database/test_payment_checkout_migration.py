...
migration = importlib.import_module(
    "src.database.alembic.versions.c4d90271a6bf_payment_checkouts"
)
...
def test_payment_migration_upgrade_and_downgrade_preserve_existing_data(monkeypatch):
    ...
    with engine.begin() as connection:
        ...
        assert (
            connection.execute(
                text("SELECT currency FROM payments WHERE id = 1")
            ).scalar_one()
            == "usd"
        )
        ...
        assert (
            connection.execute(
                text("SELECT external_payment_id FROM payments WHERE id = 1")
            ).scalar_one()
            == "pi_old"
        )
    engine.dispose()
...
def test_entire_migration_chain_matches_payment_models():
    engine = create_engine("sqlite://")
    with engine.begin() as connection:
        # Compare enum CHECK constraints directly:
        # Alembic treats type-bound checks differently.
        context = MigrationContext.configure(
            connection,
            opts={
                "include_object": lambda obj, name, type_, reflected, compared: type_
                != "check_constraint"
            }
        )
        with Operations.context(context):
            for name in [
                "0117b21577ed_initial_migration",
                "2b6e0f8a4c91_seed_default_user_groups",
                "7a92c1d4e608_backfill_user_profiles",
                "c4d90271a6bf_payment_checkouts",
                "e7a51092bc34_unique_order_movies"
            ]:
                importlib.import_module(
                    f"src.database.alembic.versions.{name}"
                ).upgrade()
        payment_differences = [
            difference
            for difference in compare_metadata(context, Base.metadata)
            if any(
                    table in str(difference)
                    for table in ["payments", "payment_items", "payment_checkouts"]
            )
        ]
        ...
        for name in ["payments", "payment_items", "payment_checkouts"]:
            actual = {
                ...
                for item in inspect(connection).get_check_constraints(name)
            }
            expected = {
                constraint.name: str(
